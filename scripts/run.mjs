...
assert(
  ['compare', 'original', 'fast'].includes(mode) &&
...
try {
  execFileSync(
    'rg',
    ['--sort', 'path', '--no-heading', '--line-number', '.', 'packages', '-g', '*.ts'],
    { cwd: pi, stdio: ['ignore', fd, 'pipe'] },
  );
} finally {
  closeSync(fd);
}
...
const sequence = Array.from({ length: runs }, () =>
  mode === 'compare' ? ['original', 'fast'] : [mode],
).flat();
let referenceScreen;
...
for (const [index, current] of sequence.entries()) {
  ...
  const variables = {
    ...
    PI_BENCH_MODE: current,
    PI_BENCH_READY: ready,
    PI_BENCH_METRICS: metrics,
    PI_BENCH_RUN: String(index + 1),
  };
  ...
  try {
    ...
    const normalized = screen
      .replaceAll(row.fullOutputPath, '<full-output>')
      .replaceAll(/pi-bash-[a-f0-9]+\.log/g, '<output.log>');
...
writeFileSync(
  join(destination, 'runs.jsonl'),
  sequence
    .map((_, index) => readFileSync(join(destination, String(index + 1), 'metrics.jsonl'), 'utf8'))
    .join(''),
);
console.log(
  execFileSync(
    process.execPath,
    [join(root, 'scripts/summarize.mjs'), join(destination, 'runs.jsonl')],
    { encoding: 'utf8' },
  ),
);
