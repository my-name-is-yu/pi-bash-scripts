...
assert.equal(
  ...
  'b46a9d0b96fb40d2065ee4da67d6247e890b9d66',
  'Target component differs from the measured source',
);
...
if (!existsSync(join(work, 'node_modules')))
  symlinkSync(join(pi, 'node_modules'), join(work, 'node_modules'), 'dir');
...
for (const [before, after] of [
  ...
  [
    ...
    'this.displayTail = (this.displayTail + clean).slice(-2 * DEFAULT_MAX_BYTES);\n\t\t// Append to output lines',
  ],
...
fast = fast.replace(
  /from "(\.[^"]+)"/g,
  (_, path) => `from ${JSON.stringify(pathToFileURL(resolve(pi, dirname(relative), path)).href)}`,
);
...
for (const name of ['instrument.mts', 'parity.mts']) {
  ...
  writeFileSync(
    join(work, name),
    template.replaceAll('__PI_SOURCE__', pathToFileURL(join(pi, 'packages/coding-agent/src')).href),
  );
}
writeFileSync(
